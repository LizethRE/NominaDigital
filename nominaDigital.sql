...
-- mar 21 nov 2017 10:20:22 -05
-- Model: liquidacionNomina    Version: 1.0
...
-- -----------------------------------------------------
-- Schema liquidacionNomina
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `liquidacionNomina` ;

-- -----------------------------------------------------
-- Schema liquidacionNomina
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `liquidacionNomina` ;
SHOW WARNINGS;
USE `liquidacionNomina` ;
...
SHOW WARNINGS;
CREATE FULLTEXT INDEX `idx_title_description` ON `film_text` (`title` ASC, `description` ASC);
...
CREATE TABLE IF NOT EXISTS `Periodo` (
  ...
  `tipo_periodo` VARCHAR(45) NULL,
  `fecha_desde` DATE NULL,
  `fecha_hasta` DATE NULL,
  `Concepto_id_concepto` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`id_Periodo`, `Concepto_id_concepto`))
ENGINE = InnoDB;

SHOW WARNINGS;
CREATE UNIQUE INDEX `id_Periodo_UNIQUE` ON `Periodo` (`id_Periodo` ASC);
...
CREATE TABLE IF NOT EXISTS `liquidacion` (
  `id` INT NULL,
  `salarioMinino` DOUBLE NULL,
  `idEmpleado` VARCHAR(15) NOT NULL,
  `idPeriodo` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`id`, `idEmpleado`, `idPeriodo`))
ENGINE = InnoDB
AUTO_INCREMENT = 01;
...
CREATE TABLE IF NOT EXISTS `contrato` (
  ...
  PRIMARY KEY (`numero`, `idTipo`, `idEmpleado`, `idCargo`))
ENGINE = InnoDB
AUTO_INCREMENT = 01;
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `idTipo` INT NOT NULL,
  PRIMARY KEY (`usuario`, `idTipo`))
ENGINE = InnoDB;

SHOW WARNINGS;

-- -----------------------------------------------------
-- Table `liquidacionConcepto`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `liquidacionConcepto` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `liquidacionConcepto` (
  `idLiquidacion` INT NOT NULL,
  `idConcepto` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`idLiquidacion`, `idConcepto`))
ENGINE = InnoDB;
...
CREATE PROCEDURE `liquidacionNomina`.`rewards_report` (
...
CREATE FUNCTION `liquidacionNomina`.`get_customer_balance`(p_customer_id INT, p_effective_date DATETIME) RETURNS DECIMAL(5,2)
    DETERMINISTIC
    READS SQL DATA
BEGIN
...
CREATE PROCEDURE `liquidacionNomina`.`film_in_stock`(IN p_film_id INT, IN p_store_id INT, OUT p_film_count INT)
READS SQL DATA
BEGIN
     SELECT inventory_id
     FROM inventory
...
CREATE PROCEDURE `liquidacionNomina`.`film_not_in_stock`(IN p_film_id INT, IN p_store_id INT, OUT p_film_count INT)
READS SQL DATA
BEGIN
     SELECT inventory_id
     FROM inventory
...
CREATE FUNCTION `liquidacionNomina`.`inventory_held_by_customer`(p_inventory_id INT) RETURNS INT
READS SQL DATA
BEGIN
...
CREATE FUNCTION `liquidacionNomina`.`inventory_in_stock`(p_inventory_id INT) RETURNS BOOLEAN
READS SQL DATA
BEGIN
...
GROUP_CONCAT(DISTINCT CONCAT(c.name, ': ',
		(SELECT GROUP_CONCAT(f.title ORDER BY f.title SEPARATOR ', ')
                    FROM liquidacionNomina.film f
                    INNER JOIN liquidacionNomina.film_category fc
                      ON f.film_id = fc.film_id
                    INNER JOIN liquidacionNomina.film_actor fa
...
AS film_info
FROM liquidacionNomina.actor a
LEFT JOIN liquidacionNomina.film_actor fa
  ON a.actor_id = fa.actor_id
LEFT JOIN liquidacionNomina.film_category fc
  ON fa.film_id = fc.film_id
LEFT JOIN liquidacionNomina.category c
